...
import '../widgets/import_my_funds_dialog.dart';
import '../widgets/expandable_search_box.dart';
import '../widgets/mobile_header.dart';
import '../widgets/copyable_text.dart';
import '../widgets/unoptimized_badge.dart';

class MyFundsTab extends StatefulWidget {
  const MyFundsTab({super.key});

  @override
  State<MyFundsTab> createState() => _MyFundsTabState();
}

class _MyFundsTabState extends State<MyFundsTab> {
    ...
    fluent.showDialog(
      ...
      builder: (context) {
        return fluent.ContentDialog(
          title: const Text('批量删除自选确认'),
          content: Text('确定要将这 ${_selectedCodes.length} 只基金从自选列表中删除吗？'),
          actions: [
            fluent.Button(
              child: const Text('取消'),
  ...
  void initState() {
    ...
    _searchFocusSub = fundProvider.searchFocusStream.listen((_) {
      if (mounted && fundProvider.currentTabIndex == 2) {
  ...
  Widget build(BuildContext context) {
    ...
    final List<FundUIModel> allList = fundProvider.myFunds.values.toList();
    final int otcCount = allList.where((m) => !m.isExchangeTraded).length;
    final int etfCount = allList.where((m) => m.isExchangeTraded).length;

    final List<FundUIModel> list = allList.where((model) {
    ...
    final Widget moreBtn = fluent.DropDownButton(
      ...
      items: [
        ...
        fluent.MenuFlyoutItem(
          ...
          text: const Text('添加自选', style: TextStyle(fontSize: 12)),
          onPressed: () async {
            Navigator.pop(context);
            final result = await fluent.showDialog<Map<String, dynamic>>(
              context: context,
              builder: (context) => const ImportMyFundsDialog(),
            );
            if (result != null &&
    ...
    final Widget deleteSelectedBtn = _selectedCodes.isEmpty
        ? const fluent.Button(
            onPressed: null,
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                ...
                SizedBox(width: 4),
                Text('批量删除 (0)', style: TextStyle(fontSize: 12)),
              ],
            ),
          )
        : fluent.Button(
            ...
            child: Row(
              ...
              children: [
                ...
                Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    _buildFilterChip('全部(${allList.length})', 0, isDark),
                    const SizedBox(width: 4),
    ...
    return fluent.ScaffoldPage(
      ...
      content: Column(
        ...
        children: [
          ...
          Expanded(
            child: fundProvider.myFunds.isEmpty
                ? Padding(
            ...
            switch (cleanTitle) {
              ...
              default:
                ...
                // 动态月份百分位列判定
                  ...
                  if (val >= 0) {
                    if (val <= 20.0) {
                      cellBg = ThemeColors.getLowPercentileBg(isDark); // 低估淡绿底
                      textColor = ThemeColors.getLowPercentileText(isDark);
                    } else if (val >= 80.0) {
                      cellBg = ThemeColors.getHighPercentileBg(isDark); // 高估淡红底
